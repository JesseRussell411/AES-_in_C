...
inline uint8_t finiteFieldMultiply(uint8_t a, uint8_t b){
...
void ShiftRowsReverse(uint8_t *state)
{
    // 00 04 08 12              \     00 04 08 12
...
void mixColumnsReverse(uint8_t *state) {
...
void AES_encryption(uint8_t *message, uint8_t *key)
{
    ...
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, expandedKey + 16 * (numberOfRounds + 1));
